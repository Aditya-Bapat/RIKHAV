...
#             # Calculate local minima from date of new_local_maxima
#             minima_between_new_max = minima[(minima > peak_index)]
#             if minima_between_new_max.size > 0:
#                 min_index_new_max = minima_between_new_max[0]  # Take the first minima after the new local maxima
#                 new_local_minima = df.iloc[min_index_new_max]['Total Cash']
#                 new_local_minima_date = df.iloc[min_index_new_max]['Date']
#             else:
#                 continue  # No minima found after the new local maxima
...
#             minima_between_old_max = minima[(minima > peak_index)]
#             if minima_between_old_max.size > 0:
#                 min_index_old_max = minima_between_old_max[0]  # Take the first minima after the old local maxima
#                 new_local_minima = df.iloc[min_index_old_max]['Total Cash']
#                 new_local_minima_date = df.iloc[min_index_old_max]['Date']
#             else:
#                 continue  # No minima found after the old local maxima

#         total_cash_difference = new_local_maxima - new_local_minima
#         # Store the updated local maxima and minima
#         paired_data.append({
#             'Peak Date': new_local_maxima_date,
#             'New Local Maxima': new_local_maxima,
#             'Peak Close Price': df.iloc[peak_index]['Close Price'],
#             'Minima Date': new_local_minima_date,
#             # 'New Local Minima': new_local_minima,
#             'New Local Minima': new_local_minima,
#             'Minima Close Price': df.iloc[min_index]['Close Price'],
#             'Total Cash Diff': total_cash_difference
#         })
        
#         # Update previous maxima and minima for the next iteration
#         prev_local_maxima = new_local_maxima
#         prev_local_minima = new_local_minima
#         prev_local_maxima_date = new_local_maxima_date
#         prev_local_minima_date = new_local_minima_date
